...
<div x-data="{ selectedCategory: 'all' }">
  <div class="flex justify-center gap-2">
    <button @click="selectedCategory = 'all'" :class="selectedCategory === 'all' ? 'bg-[#C85A32] text-white' : 'bg-[#FAF7F2] text-[#2C221E] hover:bg-white border border-[#D4A373]/30'" class="px-4 py-2 rounded-none text-xs font-semibold transition-all">
      Tous
    </button>
    @foreach($categories as $category)
      <button @click="selectedCategory = '{{ $category->slug }}'" :class="selectedCategory === '{{ $category->slug }}' ? 'bg-[#C85A32] text-white' : 'bg-[#FAF7F2] text-[#2C221E] hover:bg-white border border-[#D4A373]/30'" class="px-4 py-2 rounded-none text-xs font-semibold transition-all">
        {{ $category->libelle }}
      </button>
    @endforeach
  </div>
