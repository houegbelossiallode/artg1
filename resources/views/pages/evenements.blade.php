...
<section class="pt-32 pb-24 bg-[#F4EFE6] relative overflow-hidden" id="events" x-data="{ activeGallery: null, currentImageIndex: 0, showTitle: '' }">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
    ...
    @if($featuredEvent)
      ...
      @endphp
      <!-- Événement Phare -->
      <div class="bg-[#1E1613] text-[#FAF7F2] rounded-none overflow-hidden shadow-2xl border border-[#0BA20B]/40 grid grid-cols-1 lg:grid-cols-12">
        <div class="lg:col-span-7 p-8 sm:p-12 space-y-6 flex flex-col justify-between">
          <div class="space-y-4">
            <div class="flex items-center gap-2">
              <span class="bg-[#0BA20B] text-white text-[11px] font-bold uppercase px-3 py-1 rounded-none">
                À la une
              </span>
              <span class="text-xs text-[#0BA20B] font-semibold">
          ...
          <div class="pt-6 border-t border-white/10 flex items-center justify-between">
            ...
            <button type="button" 
                    ...
                    class="text-xs font-bold text-[#0BA20B] hover:text-white flex items-center gap-1 transition-colors">
        ...
        <div class="lg:col-span-5 relative min-h-[300px] cursor-pointer group overflow-hidden bg-black"
          ...
          <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <span class="bg-[#1E1613]/80 backdrop-blur-sm text-[#FAF7F2] border border-[#0BA20B]/40 px-4 py-2 text-xs uppercase tracking-widest font-bold">
              Ouvrir la galerie
            </span>
          </div>
        </div>
      ...
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          @foreach($upcomingEvents as $evt)
            ...
            @endphp
            <div class="bg-[#FAF7F2] rounded-none overflow-hidden border border-[#0BA20B]/30 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between group">
              <div>
                <div class="relative h-48 overflow-hidden bg-black cursor-pointer"
                  ...
                  <span class="absolute top-3 left-3 bg-[#1E1613]/80 text-[#0BA20B] text-[10px] font-bold px-2.5 py-0.5 rounded-none backdrop-blur-sm">
                ...
                <div class="p-5 space-y-3">
                  ...
                  <div class="space-y-1 text-xs text-[#8C766B] pt-2 border-t border-[#0BA20B]/20">
                    <p class="flex items-center gap-1.5">
                      <svg aria-hidden="true" class="lucide lucide-map-pin w-3.5 h-3.5 text-[#0BA20B]" fill="none" height="24" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"></path><circle cx="12" cy="10" r="3"></circle></svg>
                      <span class="line-clamp-1">{{ $evt->lieu }}</span>
                    </p>
                    <p class="flex items-center gap-1.5">
                      <svg aria-hidden="true" class="lucide lucide-clock w-3.5 h-3.5 text-[#0BA20B]" fill="none" height="24" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewbox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M12 6v6l4 2"></path><circle cx="12" cy="12" r="10"></circle></svg>
    ...
    <div x-show="activeGallery" 
        ...
        <div class="relative w-full max-w-5xl bg-[#1E1613] border border-[#0BA20B]/30 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden rounded-none"
        ...
        <div class="relative flex-1 bg-black flex items-center justify-center min-h-[350px] p-4 overflow-hidden">
            ...
            <div>
                <button @click="currentImageIndex = (currentImageIndex === 0) ? activeGallery.length - 1 : currentImageIndex - 1" 
                        class="absolute left-4 top-1/2 -translate-y-1/2 bg-[#1E1613]/80 hover:bg-[#0BA20B] text-[#FAF7F2] p-3 transition-colors border border-[#0BA20B]/30 shadow-xl rounded-none group">
                ...
                <button @click="currentImageIndex = (currentImageIndex === activeGallery.length - 1) ? 0 : currentImageIndex + 1" 
                        class="absolute right-4 top-1/2 -translate-y-1/2 bg-[#1E1613]/80 hover:bg-[#0BA20B] text-[#FAF7F2] p-3 transition-colors border border-[#0BA20B]/30 shadow-xl rounded-none group">
        ...
        <template x-if="activeGallery && activeGallery.length > 1">
            ...
            <template x-for="(img, idx) in activeGallery" :key="idx">
                <button @click="currentImageIndex = idx" 
                        ...
                        :class="currentImageIndex === idx ? 'border-[#0BA20B] scale-105 opacity-100 z-10' : 'border-[#0BA20B]/30 opacity-50 hover:opacity-100'">
