...
<section class="pt-32 pb-24 bg-[#FAF7F2]" x-data="{ activeGallery: null, currentImageIndex: 0, showTitle: '' }">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div class="text-center max-w-4xl mx-auto space-y-6 pt-8">
            <div class="inline-flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.2em] text-[#C85A32]">
                <a href="{{ url('/') }}" class="hover:text-[#A84223] transition-colors">Accueil</a>
                <span class="text-[#D4A373]">-</span>
                @if($type === 'evenement')
                    <a href="{{ route('evenements') }}" class="hover:text-[#A84223] transition-colors">Événements</a>
                @else
                    <a href="{{ route('actualites') }}" class="hover:text-[#A84223] transition-colors">Actualités</a>
                @endif
                <span class="text-[#D4A373]">-</span>
            ...
            @if($type === 'evenement')
                <div class="flex items-center justify-center gap-2 pt-4">
                    <span class="bg-[#1E1613] text-[#D4A373] px-4 py-1.5 text-xs font-bold uppercase tracking-widest">
        ...
        <!-- Featured Image -->
        <div class="w-full h-[400px] sm:h-[500px] overflow-hidden border border-[#D4A373]/30 shadow-sm relative group">
        ...
        <div class="flex flex-col lg:flex-row gap-12 lg:gap-16 items-start">
            ...
            <div class="w-full lg:w-2/3 space-y-12">
                <div class="prose prose-lg prose-[#6B574F] max-w-none font-sans leading-loose prose-headings:font-serif-title prose-headings:text-[#1E1613] prose-a:text-[#C85A32] hover:prose-a:text-[#A84223]">
                    @if($type === 'evenement')
                        {!! $item->description !!}
                    @else
                        {!! $item->contenu !!}
                ...
                <!-- Gallery -->
                <div class="pt-12 border-t border-[#D4A373]/20">
                    <h3 class="font-serif-title text-2xl font-bold text-[#1E1613] mb-8">Galerie Associée</h3>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        @foreach($item->images as $img)
                            <div class="relative h-32 sm:h-48 overflow-hidden bg-[#1E1613] cursor-pointer group border border-[#D4A373]/20"
                                ...
                                @if($img->is_principal)
                                <span class="absolute top-2 left-2 bg-[#C85A32] text-white text-[9px] font-bold px-2 py-0.5 uppercase tracking-wider">À la une</span>
                                @endif
                                <div class="absolute inset-0 bg-[#C85A32]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="w-full lg:w-1/3 sticky top-32">
                <div class="bg-white border border-[#D4A373]/20 p-8 shadow-sm relative">
                    <!-- Deco corners -->
                    <div class="absolute top-0 left-0 w-2 h-2 border-t border-l border-[#C85A32]"></div>
                    <div class="absolute top-0 right-0 w-2 h-2 border-t border-r border-[#C85A32]"></div>
                    <div class="absolute bottom-0 left-0 w-2 h-2 border-b border-l border-[#C85A32]"></div>
                    <div class="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-[#C85A32]"></div>

                    <h3 class="font-serif-title text-xl font-bold text-[#1E1613] mb-6 pb-4 border-b border-[#D4A373]/20 uppercase tracking-widest text-center">
                        @if($type === 'evenement')
                        L'Événement
                        @else
                        L'Article
                    ...
                    <ul class="space-y-6">
                        @if($type === 'evenement')
                            <li class="flex items-start gap-4">
                                <div class="mt-1">
                                    <svg class="w-4 h-4 text-[#C85A32]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                </div>
                                <div>
                                    <p class="text-[9px] uppercase font-bold text-[#D4A373] tracking-widest mb-0.5">Date</p>
                            ...
                            <li class="flex items-start gap-4">
                                <div class="mt-1">
                                    <svg class="w-4 h-4 text-[#C85A32]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                </div>
                                <div>
                                    <p class="text-[9px] uppercase font-bold text-[#D4A373] tracking-widest mb-0.5">Heure</p>
                            ...
                            <li class="flex items-start gap-4">
                                ...
                                <div>
                                    <p class="text-[9px] uppercase font-bold text-[#D4A373] tracking-widest mb-0.5">Lieu</p>
                                    <p class="text-[#2C221E] font-medium text-sm">{{ $item->lieu }}</p>
                                </div>
                            </li>

                            <li class="pt-6 mt-2 border-t border-[#D4A373]/20">
                                <button class="w-full py-3.5 px-6 bg-[#C85A32] hover:bg-[#A84223] text-white font-bold tracking-widest uppercase text-xs transition-colors flex items-center justify-center gap-2">
                                    Réserver
                        ...
                        @else
                            <li class="flex items-start gap-4">
                                ...
                                <div>
                                    <p class="text-[9px] uppercase font-bold text-[#D4A373] tracking-widest mb-0.5">Publié le</p>
                            ...
                            <li class="flex items-start gap-4">
                                <div class="mt-1">
                                    <svg class="w-4 h-4 text-[#C85A32]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                </div>
                                <div>
                                    <p class="text-[9px] uppercase font-bold text-[#D4A373] tracking-widest mb-0.5">Par</p>
    ...
    <div x-show="activeGallery" 
        ...
        <div class="relative w-full max-w-5xl bg-[#1E1613] border border-[#D4A373]/30 shadow-2xl flex flex-col max-h-[95vh] overflow-hidden rounded-none"
        ...
        <div class="relative flex-1 bg-black flex items-center justify-center min-h-[400px] p-4 overflow-hidden">
            ...
            <div>
                <button @click="currentImageIndex = (currentImageIndex === 0) ? activeGallery.length - 1 : currentImageIndex - 1" 
                        class="absolute left-6 top-1/2 -translate-y-1/2 bg-[#1E1613]/90 hover:bg-[#C85A32] text-[#FAF7F2] p-4 transition-colors border border-[#D4A373]/30 shadow-xl rounded-none group">
                ...
                <button @click="currentImageIndex = (currentImageIndex === activeGallery.length - 1) ? 0 : currentImageIndex + 1" 
                        class="absolute right-6 top-1/2 -translate-y-1/2 bg-[#1E1613]/90 hover:bg-[#C85A32] text-[#FAF7F2] p-4 transition-colors border border-[#D4A373]/30 shadow-xl rounded-none group">
        ...
        <template x-if="activeGallery && activeGallery.length > 1">
            <div class="px-6 py-4 bg-[#1E1613] border-t border-[#D4A373]/20 flex items-center justify-center gap-4 overflow-x-auto">
            <template x-for="(img, idx) in activeGallery" :key="idx">
                <button @click="currentImageIndex = idx" 
                        ...
                        :class="currentImageIndex === idx ? 'border-[#C85A32] scale-110 opacity-100 shadow-lg' : 'border-[#D4A373]/30 opacity-60 hover:opacity-100'">
