...
<header class="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-6 md:px-10 z-30 sticky top-0 shadow-sm">
  ...
  <div class="flex items-center gap-6">
    ...
    <div class="relative" x-data="{ open: false }" @click.away="open = false">
      <button @click="open = !open" class="relative text-slate-400 hover:text-slate-900 transition-colors focus:outline-none">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
        <span class="absolute top-0 right-0 w-2 h-2 bg-brand-lime rounded-full"></span>
      </button>
      ...
      <div x-show="open" 
           ...
           class="absolute right-0 mt-2 w-80 bg-white border border-slate-200 shadow-xl z-50 overflow-hidden" 
           style="display: none;">
        <div class="bg-slate-50 border-b border-slate-200 px-4 py-3 flex justify-between items-center">
          ...
          <a href="#" class="text-[10px] text-brand-lime font-bold hover:text-amber-700">Marquer tout lu</a>
        </div>
        <div class="divide-y divide-slate-100 max-h-64 overflow-y-auto">
          <a href="#" class="block px-4 py-3 hover:bg-slate-50 transition-colors">
            <p class="text-sm font-bold text-slate-900">Nouveau cours publié</p>
            <p class="text-xs text-slate-500 mt-1 line-clamp-1">Le support pour "Harmonie Classique" est disponible.</p>
            <span class="text-[10px] text-slate-400 mt-2 block">Il y a 5 min</span>
          </a>
          <a href="#" class="block px-4 py-3 hover:bg-slate-50 transition-colors">
            <p class="text-sm font-bold text-slate-900">Paiement validé</p>
            <p class="text-xs text-slate-500 mt-1 line-clamp-1">Votre facture de 120€ a bien été traitée.</p>
            <span class="text-[10px] text-slate-400 mt-2 block">Il y a 2h</span>
          </a>
        </div>
      </div>
    </div>
