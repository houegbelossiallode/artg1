    ...
    <div class="bg-white border border-slate-200 p-4 flex flex-wrap gap-2 items-center justify-between">
        <div class="flex flex-wrap gap-2">
            <button type="button" @click="activeCategory = 'Tous'"
                :class="activeCategory === 'Tous' ? 'bg-[#C85A32] text-white border-[#C85A32]' : 'bg-[#FAF7F2] text-[#2C221E] border-[#D4A373]/40 hover:bg-[#F4EFE6]'"
            ...
            @foreach($categoriesCours as $cat)
                <button type="button" @click="activeCategory = '{{ addslashes($cat->nom) }}'"
                    :class="activeCategory === '{{ addslashes($cat->nom) }}' ? 'bg-[#C85A32] text-white border-[#C85A32]' : 'bg-[#FAF7F2] text-[#2C221E] border-[#D4A373]/40 hover:bg-[#F4EFE6]'"
                    class="px-3 py-1.5 text-xs font-bold uppercase tracking-wider border transition">
                    {{ $cat->nom }}
                </button>
            @endforeach
            ...
            <div x-show="(activeCategory === 'Tous' || activeCategory === '{{ addslashes($item->categorie ? $item->categorie->nom : '') }}') && (activeMode === 'Tous' || activeMode === '{{ addslashes($modeName) }}')"
                ...
                <div class="relative h-44 overflow-hidden bg-slate-100">
                    ...
                    <div class="absolute top-3 left-3 flex flex-wrap gap-1.5">
                        <span class="px-2.5 py-1 bg-[#1E1613]/90 backdrop-blur-sm text-[#D4A373] text-[9px] font-bold uppercase tracking-widest border border-[#D4A373]/30">
                            {{ $item->categorie ? $item->categorie->nom : 'Général' }}
                        </span>
                        <span class="px-2.5 py-1 bg-[#C85A32] text-white text-[9px] font-bold uppercase tracking-widest shadow-sm">
                            {{ $modeName }}
                        </span>
                    </div>
                </div>
                <div class="p-5 flex-1 flex flex-col justify-between space-y-4">
                    <div>
                        <h3 class="text-base font-serif-title font-bold text-slate-900 group-hover:text-[#C85A32] transition-colors">
                            {{ $item->titre }}
                        </h3>
                    ...
                    <div class="pt-3 border-t border-slate-100 flex items-center justify-between">
                        <div>
                            ...
                            <span class="text-base font-bold text-[#C85A32]">
                        ...
                        @else -->
                            <button type="button"
                                ...
                                class="px-4 py-2 bg-[#C85A32] hover:bg-[#A84223] text-white text-xs font-bold uppercase tracking-wider transition shadow cursor-pointer">
                                Réserver
                            </button>
                        <!-- @endif -->
                    </div>
            ...
            <div class="flex justify-between items-start border-b border-[#D4A373]/20 pb-2.5 mb-3">
                <div>
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span x-show="coursTarif" class="text-[9px] font-bold uppercase px-2 py-0.5 bg-[#C85A32] text-white" x-text="coursTarif + ' € / séance'"></span>
                    </div>
            ...
            <div x-show="!loading">
                <div class="mb-3 bg-[#F4EFE6] border border-[#D4A373]/30 p-2.5">
                    <div class="flex items-center justify-between mb-1.5">
                        ...
                        <span x-show="disponibilites.length > 0" class="text-[9px] font-bold text-[#C85A32] bg-[#FAF7F2] border border-[#D4A373]/40 px-2 py-0.5" x-text="disponibilites.length + ' créneau(x)'"></span>
                    </div>
                    ...
                    <div x-show="disponibilites.length > 0" class="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                        <template x-for="(slots, jour) in getGroupedDisponibilites()" :key="jour">
                            <div class="bg-white border border-[#D4A373]/30 p-2 shadow-sm">
                                <div class="text-[10px] font-bold text-[#C85A32] uppercase mb-1 flex items-center justify-between border-b border-[#D4A373]/20 pb-1">
                                    <div class="flex items-center gap-1.5">
                                        <span class="w-2 h-2 bg-emerald-500 rounded-full inline-block"></span>
                                        <span x-text="jour + (getSlotNextDateFormatted(jour) ? ' (' + getSlotNextDateFormatted(jour) + ')' : '')"></span>
                                    </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                                    <template x-for="slot in slots" :key="slot.id">
                                        <button type="button" @click="selectSlot(slot)"
                                            ...
                                            <span class="text-[9px] font-bold uppercase" :class="dateReservation === getNextDateForDay(slot.jour) && heureDebut === slot.debut.substring(0,5) ? 'text-white' : 'text-[#C85A32]'">✓ Choisir</span>
                                        </button>
                                    </template>
                                </div>
                            </div>
                ...
                <form action="{{ route('reservations.store') }}" method="POST" @submit="validateForm($event)" class="space-y-3">
                    ...
                    <div class="flex justify-end gap-2.5 pt-2 border-t border-[#D4A373]/20">
                        ...
                        <button type="submit" :disabled="disponibilites.length === 0" class="px-4 py-2 bg-[#C85A32] hover:bg-[#A84223] disabled:opacity-50 text-white font-bold text-xs uppercase tracking-widest transition shadow-md cursor-pointer">
