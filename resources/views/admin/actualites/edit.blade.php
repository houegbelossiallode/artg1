...
@section('title', 'Modifier une Actualité | AssoCulture')
...
<div class="space-y-6">
  ...
  <form action="{{ route('dashboard.admin.actualites.update', $actualite->id) }}" method="POST" enctype="multipart/form-data" class="bg-white border border-slate-200 shadow-sm p-6 space-y-6" x-data="editForm()">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 space-y-4">
        <div>
          ...
          <input type="text" name="titre" required class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none bg-slate-50" placeholder="Titre de l'article" value="{{ old('titre', $actualite->titre) }}">
        ...
        <div>
          ...
          <textarea name="contenu" rows="8" required class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none bg-slate-50" placeholder="Texte complet de l'article...">{{ old('contenu', $actualite->contenu) }}</textarea>
      ...
      <div class="space-y-4">
        <div>
          ...
          <input type="date" name="date_publication" required class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none bg-slate-50" value="{{ old('date_publication', \Carbon\Carbon::parse($actualite->date_publication)->format('Y-m-d')) }}">
          @error('date_publication') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
        </div>

        @php
            $mainImage = $actualite->images->where('is_principal', true)->first();
            $mainImageUrl = $mainImage ? asset('storage/' . $mainImage->image_path) : null;
        @endphp

        <!-- DROPZONE AlpineJS for Main Image -->
        <div x-data="singleImageUploader('{{ $mainImageUrl }}', {{ $mainImage ? $mainImage->id : 'null' }})">
    ...
    <div class="border-t border-slate-200 pt-6">
      ...
      @php
          $secondaryImages = $actualite->images->where('is_principal', false);
      @endphp

      @if($secondaryImages->count() > 0)
        <div class="mb-6">
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Images Secondaires Existantes</label>
      ...
      <!-- DROPZONE AlpineJS for Adding Multiple New Images -->
      <div x-data="multipleImageUploader()">
        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Ajouter de nouvelles images secondaires</label>
        ...
        >
          ...
          <div class="pointer-events-none">
            ...
            <p class="text-xs text-slate-500 font-medium">Glissez plusieurs nouvelles images ici ou <span class="text-slate-900 underline">cliquez</span> pour parcourir</p>
          </div>
        </div>
...
<script>
  ...
  function singleImageUploader(initialUrl = null, imageId = null) {
    return {
      dragover: false,
      ...
      removeImage() {
        ...
        // If there was an initial image, mark it for deletion
        if (imageId) {
          // This accesses the parent Alpine component data
          if (typeof this.$data.deletedImages !== 'undefined') {
            this.$data.deletedImages.push(imageId);
          }
        }
      }
    }
