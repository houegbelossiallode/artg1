...
@section('title', 'Gestion des Sous-menus | AssoCulture')
...
<div class="space-y-6" x-data="{ createModalOpen: false, editModalOpen: false, editSousmenu: {}, updateUrl: '' }">
    ...
    <div class="overflow-x-auto overflow-y-visible min-h-[300px]">
      <table class="w-full text-left text-sm text-slate-600">
        <thead class="border-b border-slate-200 text-[10px] uppercase tracking-widest text-slate-400 font-bold bg-slate-50">
          <tr>
            <th class="p-4">Sous-menu</th>
            <th class="p-4">Menu Parent</th>
            <th class="p-4">URL</th>
        ...
        <tbody class="divide-y divide-slate-100">
          @forelse ($sousmenus as $sub)
            <tr class="hover:bg-slate-50 transition-colors">
              <td class="p-4 font-bold text-slate-900 flex items-center gap-2">
                <span class="w-6 h-6 bg-slate-100 border border-slate-200 inline-flex items-center justify-center text-[10px] font-mono text-slate-400">SM</span>
                {{ $sub->libelle ?? $sub->nom }}
              </td>
              <td class="p-4">
                @if($sub->menu)
              ...
              <td class="p-4 text-right">
                ...
                <div class="relative inline-block text-left" x-data="{ open: false }" @click.away="open = false">
                  ...
                  <div x-show="open" 
                    ...
                    <div class="py-1">
                      <form action="{{ route('dashboard.admin.sousmenus.destroy', $sub->id) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer ce sous-menu ?');">
                        @csrf
                        @method('DELETE')
          ...
          @empty
            <tr>
              <td colspan="5" class="p-8 text-center text-xs text-slate-400">Aucun sous-menu enregistré.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
  ...
  <div x-show="createModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak>
    <div @click.away="createModalOpen = false" class="bg-white max-w-lg w-full rounded-none shadow-2xl border border-slate-200 overflow-hidden">
      ...
      <form action="{{ route('dashboard.admin.sousmenus.store') }}" method="POST" class="p-6 space-y-4">
        @csrf
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Menu Parent *</label>
          <select name="menu_id" required class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none bg-white">
            <option value="">-- Sélectionner un menu parent --</option>
            @foreach($menus as $m)
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Libellé du Sous-menu *</label>
          <input type="text" name="libelle" required placeholder="ex: Calendrier des cours" class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none">
        </div>

        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">URL / Route *</label>
          <input type="text" name="url" required placeholder="/admin/cours/calendrier" class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none">
        </div>
  ...
  <div x-show="editModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak>
    <div @click.away="editModalOpen = false" class="bg-white max-w-lg w-full rounded-none shadow-2xl border border-slate-200 overflow-hidden">
      ...
      <form :action="updateUrl" method="POST" class="p-6 space-y-4">
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Menu Parent *</label>
          <select name="menu_id" x-model="editSousmenu.menu_id" required class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none bg-white">
            @foreach($menus as $m)
              <option value="{{ $m->id }}">{{ $m->libelle ?? $m->nom }}</option>
            @endforeach
          </select>
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Libellé du Sous-menu *</label>
          <input type="text" name="libelle" x-model="editSousmenu.libelle" required class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none">
        </div>

        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">URL / Route *</label>
          <input type="text" name="url" x-model="editSousmenu.url" required class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none">
        </div>
