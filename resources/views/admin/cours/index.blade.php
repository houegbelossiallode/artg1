...
<div class="space-y-6" x-data="{ createModalOpen: false, editModalOpen: false, editCours: {} }">
  ...
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-slate-900 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">CATALOGUE DES COURS</h1>
      <p class="text-slate-400 text-sm mt-0.5">Gérez le programme des formations, horaires et professeurs assignés.</p>
    </div>
          ...
          @forelse ($cours as $item)
            <tr class="hover:bg-slate-50 transition-colors">
              ...
              <td class="p-4 text-xs font-bold text-slate-800">
                {{ $item->professeur ? $item->professeur->name : '-' }}
              </td>
              <td class="p-4">
              ...
              <td class="p-4 text-right">
                ...
                <div class="relative inline-block text-left" x-data="{ open: false }" @click.away="open = false">
                  ...
                  <div x-show="open" 
                       ...
                       x-cloak>
                    <div class="py-1">
                      <button type="button" @click="editCours = { id: {{ $item->id }}, titre: '{{ addslashes($item->titre) }}', categorie_cours_id: {{ $item->categorie_cours_id }}, user_id: {{ $item->user_id }}, mode_id: {{ $item->mode_id }}, tarif: {{ $item->tarif }}, description: '{{ addslashes($item->description ?? '') }}' }; editModalOpen = true; open = false;" class="w-full text-left group flex items-center px-4 py-2 text-xs text-slate-700 hover:bg-slate-900 hover:text-white font-bold uppercase tracking-wider">
