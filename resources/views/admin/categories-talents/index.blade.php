...
<div class="space-y-6" x-data="{ showModal: false, editMode: false, category: null, form: { nom: '' } }">
  ...
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-slate-900 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">CATÉGORIES DE TALENTS</h1>
      <p class="text-slate-400 text-sm mt-0.5">Gérez les catégories de talents (Musique, Danse, Théâtre, etc.).</p>
    </div>
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div class="sm:flex sm:items-start">
            <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
              ...
              <form :action="editMode ? '{{ route('dashboard.admin.categories-talents.update', '__ID__') }}'.replace('__ID__', category.id) : '{{ route('dashboard.admin.categories-talents.store') }}'" method="POST" class="mt-4 space-y-4">
                @csrf
