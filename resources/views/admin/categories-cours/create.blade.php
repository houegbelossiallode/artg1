...
@section('title', 'Nouvelle Catégorie de Cours | AssoCulture')

@section('content')
<div class="space-y-6">
  <!-- En-tête -->
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-slate-900">
    <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">NOUVELLE CATÉGORIE DE COURS</h1>
  </div>

  <div class="bg-white border border-slate-200 shadow-sm rounded-none p-6">
    <form action="{{ route('dashboard.admin.categories-cours.store') }}" method="POST" class="space-y-4">
      @csrf
      <div>
      ...
      <div>
        ...
        <textarea name="description" rows="3" class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none" placeholder="Description de la catégorie..."></textarea>
      </div>
