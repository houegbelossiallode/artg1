...
@section('title', 'Catégories de Cours | AssoCulture')

@section('content')
<div class="space-y-6" x-data="{ showModal: false, editMode: false, category: null, form: { nom: '', description: '' } }">
    ...
    <div class="overflow-x-auto min-h-[300px]">
      <table class="w-full text-left text-sm text-slate-600">
        ...
        <tbody class="divide-y divide-slate-100">
              ...
              <td class="p-4 text-right pr-6">
                ...
                <div class="relative inline-block text-left" x-data="{ open: false }" @click.away="open = false">
                  ...
                  <div x-show="open" 
                    ...
                    <div class="py-1">
                      <form action="{{ route('dashboard.admin.categories-cours.destroy', $category) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer cette catégorie ?');">
                        @csrf
                        @method('DELETE')
          ...
          @empty
            <tr>
              <td colspan="3" class="p-8 text-center text-xs text-slate-400">Aucune catégorie pour le moment.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
            ...
            <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
              ...
              <form :action="editMode ? '{{ route('dashboard.admin.categories-cours.update', '__ID__') }}'.replace('__ID__', category.id) : '{{ route('dashboard.admin.categories-cours.store') }}'" method="POST" class="mt-4 space-y-4">
                ...
                <div>
                  ...
                  <input type="text" name="nom" x-model="form.nom" required class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none">
                </div>
