...
@section('title', 'Gestion des Profils | AssoCulture')
...
<div class="py-1">
  ...
  <div x-show="createModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak>
    <div @click.away="createModalOpen = false" class="bg-white max-w-lg w-full rounded-none shadow-2xl border border-slate-200 overflow-hidden">
      ...
      <form action="{{ route('dashboard.admin.profils.store') }}" method="POST" class="p-6 space-y-4">
        @csrf
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Nom du Profil *</label>
          <input type="text" name="nom" required placeholder="ex: administrateur, professeur, apprenant" class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none">
        </div>
  ...
  <div x-show="editModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak>
    <div @click.away="editModalOpen = false" class="bg-white max-w-lg w-full rounded-none shadow-2xl border border-slate-200 overflow-hidden">
      ...
      <form :action="updateUrl" method="POST" class="p-6 space-y-4">
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Nom du Profil *</label>
          <input type="text" name="nom" x-model="editProfil.nom" required class="w-full px-3 py-2 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none">
        </div>
