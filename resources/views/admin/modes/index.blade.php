...
<div class="space-y-6" x-data="{ createModalOpen: false, editModalOpen: false, editMode: {}, updateUrl: '' }">
  ...
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-slate-900 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">MODES DE COURS</h1>
      <p class="text-slate-400 text-sm mt-0.5">Gérez les formats d'enseignement (Présentiel, En Ligne, Hybride, Atelier).</p>
    </div>
    ...
    <div class="overflow-x-auto overflow-y-visible min-h-[300px]">
      <table class="w-full text-left text-sm text-slate-600">
        <thead class="border-b border-slate-200 text-[10px] uppercase tracking-widest text-slate-400 font-bold bg-slate-50">
          <tr>
            <th class="p-4">ID</th>
            <th class="p-4">Mode d'Enseignement</th>
        ...
        <tbody class="divide-y divide-slate-100">
          @forelse ($modes as $mode)
            <tr class="hover:bg-slate-50 transition-colors">
              <td class="p-4 font-mono text-xs text-slate-400">#{{ $mode->id }}</td>
              <td class="p-4 font-bold text-slate-900">{{ $mode->libelle ?? $mode->nom }}</td>
              <td class="p-4">
                <span class="bg-emerald-100 text-emerald-700 text-[9px] font-bold px-2 py-0.5 uppercase tracking-widest rounded-sm">
                  {{ strtoupper($mode->actif ?? 'OUI') }}
                </span>
              </td>
              <td class="p-4 text-right">
                ...
                <div class="relative inline-block text-left" x-data="{ open: false }" @click.away="open = false">
                  ...
                  <div x-show="open" 
                       ...
                       x-cloak>
                    <div class="py-1">
                      <button type="button" @click="editMode = { id: {{ $mode->id }}, libelle: '{{ addslashes($mode->libelle ?? $mode->nom) }}' }; updateUrl = '/dashboard/admin/modes/' + {{ $mode->id }}; editModalOpen = true; open = false;" class="w-full text-left group flex items-center px-4 py-2 text-xs text-slate-700 hover:bg-slate-900 hover:text-white font-bold uppercase tracking-wider">
          ...
          @empty
            <tr>
              <td colspan="4" class="p-8 text-center text-xs text-slate-400">Aucun mode de cours enregistré.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
