...
@section('title', 'Modifier Catégorie d\'Événement | AssoCulture')

@section('content')
<div class="space-y-6">
  <!-- En-tête -->
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-slate-900">
    <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">MODIFIER LA CATÉGORIE D'ÉVÉNEMENT</h1>
  </div>

  <div class="bg-white border border-slate-200 shadow-sm rounded-none p-6">
    <form action="{{ route('dashboard.admin.categories-evenements.update', $category->id) }}" method="POST" class="space-y-4">
      @csrf
      @method('PUT')
      <div>
