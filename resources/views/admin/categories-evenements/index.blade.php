...
<div class="space-y-6" x-data="{ showModal: false, editMode: false, category: null, form: { nom: '' } }">
  ...
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-slate-900 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">CATÉGORIES D'ÉVÉNEMENTS</h1>
      <p class="text-slate-400 text-sm mt-0.5">Gérez les catégories d'événements (Concerts, Expos, Ateliers, etc.).</p>
    </div>
    <div class="flex flex-wrap items-center gap-2">
      <button @click="showModal = true; editMode = false; form = { nom: '' }" class="inline-flex items-center gap-1.5 px-4 py-2 bg-brand-lime text-slate-900 font-bold text-[10px] uppercase tracking-widest hover:brightness-105 transition shadow-sm cursor-pointer">
    ...
    <div class="overflow-x-auto min-h-[300px]">
      <table class="w-full text-left text-sm text-slate-600">
        ...
        <tbody class="divide-y divide-slate-100">
              ...
              <td class="p-4 text-right pr-6">
                ...
                <div class="relative inline-block text-left" x-data="{ open: false }" @click.away="open = false">
                  ...
                  <div x-show="open" 
                    ...
                    <div class="py-1">
                      <form action="{{ route('dashboard.admin.categories-evenements.destroy', $category) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer cette catégorie ?');">
                        @csrf
                        @method('DELETE')
          ...
          @empty
            <tr>
              <td colspan="2" class="p-8 text-center text-xs text-slate-400">Aucune catégorie pour le moment.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div class="sm:flex sm:items-start">
            <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
              ...
              <form :action="editMode ? '{{ route('dashboard.admin.categories-evenements.update', '__ID__') }}'.replace('__ID__', category.id) : '{{ route('dashboard.admin.categories-evenements.store') }}'" method="POST" class="mt-4 space-y-4">
                @csrf
                ...
                <div>
                  ...
                  <input type="text" name="nom" x-model="form.libelle" required class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none">
                </div>
