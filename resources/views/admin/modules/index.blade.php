...
<div class="space-y-6" x-data="{ createModalOpen: false, editModalOpen: false, editModule: {}, updateUrl: '' }">
  ...
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-slate-900 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">MODULES SYSTÈME</h1>
      <p class="text-slate-400 text-sm mt-0.5">Gérez les modules principaux d'accès de l'application.</p>
    </div>
        ...
        <tbody class="divide-y divide-slate-100">
          @forelse ($modules as $module)
            <tr class="hover:bg-slate-50 transition-colors">
              <td class="p-4 font-mono text-xs text-slate-400">#{{ $module->id }}</td>
              <td class="p-4 font-bold text-slate-900">{{ $module->libelle ?? $module->nom }}</td>
              <td class="p-4">
                <span class="bg-emerald-100 text-emerald-700 text-[9px] font-bold px-2 py-0.5 uppercase tracking-widest rounded-sm">
                  {{ strtoupper($module->actif ?? 'OUI') }}
                </span>
              </td>
              <td class="p-4 text-xs text-slate-400">
                {{ $module->created_at ? $module->created_at->format('d/m/Y') : '-' }}
              </td>
              <td class="p-4 text-right">
                ...
                <div class="relative inline-block text-left" x-data="{ open: false }" @click.away="open = false">
                  ...
                  <div x-show="open" 
                       ...
                       x-cloak>
                    <div class="py-1">
                      <button type="button" @click="editModule = { id: {{ $module->id }}, libelle: '{{ addslashes($module->libelle ?? $module->nom) }}', actif: '{{ $module->actif }}' }; updateUrl = '/dashboard/admin/modules/' + {{ $module->id }}; editModalOpen = true; open = false;" class="w-full text-left group flex items-center px-4 py-2 text-xs text-slate-700 hover:bg-slate-900 hover:text-white font-bold uppercase tracking-wider">
          ...
          @empty
            <tr>
              <td colspan="5" class="p-8 text-center text-xs text-slate-400">Aucun module enregistré.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
