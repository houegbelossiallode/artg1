...
@section('title', 'Gestion des Événements | AssoCulture')
...
<div class="space-y-6" x-data="{ activeGallery: null, currentImageIndex: 0, showTitle: '' }">
  ...
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-slate-900 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">ÉVÉNEMENTS & REPRÉSENTATIONS</h1>
      <p class="text-slate-400 text-sm mt-0.5">Gérez la programmation culturelle, les concerts et les ateliers.</p>
    </div>
    <div class="flex flex-wrap items-center gap-2">
      <a href="{{ route('dashboard.admin.evenements.create') }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-900 text-amber-500 font-bold text-[10px] uppercase tracking-widest hover:bg-slate-800 transition shadow-sm cursor-pointer">
  ...
  <!-- Grille d'Événements Épurée & Claire pour Dashboard -->
  @if ($evenements->count() > 0)
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      @foreach ($evenements as $evt)
        @php
          $mainImage = $evt->images->where('is_principal', true)->first();
          $allImages = $evt->images;
        ...
        <div class="bg-white border border-slate-200 shadow-sm hover:border-slate-400 transition-all flex flex-col justify-between overflow-hidden group">
          ...
          @if($mainImage)
            <div class="w-full h-44 bg-slate-100 overflow-hidden relative cursor-pointer"
                 @click="activeGallery = {{ $imagesJson }}; currentImageIndex = 0; showTitle = '{{ addslashes($evt->titre) }}'">
          ...
          <div class="p-4 space-y-2.5 flex-1 flex flex-col justify-between">
            <div>
              ...
              <div class="flex items-center justify-between text-xs text-slate-400 mb-1.5">
                ...
                <span class="px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded-none {{ strtolower($evt->actif) === 'oui' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-600' }}">
                  {{ strtolower($evt->actif) === 'oui' ? 'Actif' : 'Annulé' }}
                </span>
              </div>
          ...
          <div class="px-4 py-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
              <a href="{{ route('dashboard.admin.evenements.edit', $evt->id) }}" 
              ...
              <form action="{{ route('dashboard.admin.evenements.destroy', $evt->id) }}" method="POST" onsubmit="return confirm('Voulez-vous vraiment supprimer cet événement ?');">
                @csrf
                @method('DELETE')
  ...
  @else
    ...
    <div class="bg-white border border-slate-200 p-10 text-center space-y-3 max-w-md mx-auto my-8">
      ...
      <h3 class="text-sm font-bold text-slate-900 uppercase tracking-wider">Aucun événement</h3>
      <p class="text-xs text-slate-500">Commencez par ajouter votre premier événement à l'agenda.</p>
      <div class="pt-2">
        <a href="{{ route('dashboard.admin.evenements.create') }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-slate-900 text-amber-500 font-bold text-xs uppercase tracking-wider hover:bg-slate-800 transition">
          + CRÉER UN ÉVÉNEMENT
        </a>
      </div>
    </div>
