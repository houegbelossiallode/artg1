...
<div class="space-y-6">
  ...
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">MODIFIER L'ÉVÉNEMENT</h1>
      <p class="text-slate-400 text-sm mt-0.5">Mettez à jour les détails de cet événement.</p>
    </div>
  ...
  <form action="{{ route('dashboard.admin.evenements.update', $evenement->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="editForm()">
        ...
        <div class="bg-white border border-slate-200 shadow-sm p-6 relative overflow-hidden">
          ...
          <div class="space-y-5">
            <div>
              ...
              <input type="text" name="titre" required class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-brand-lime focus:ring-1 focus:ring-brand-lime focus:outline-none rounded-none bg-slate-50 transition-colors" placeholder="Ex: Festival des Arts Vivants" value="{{ old('titre', $evenement->titre) }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  <div class="relative">
                    ...
                    <input type="text" name="lieu" required class="w-full pl-9 pr-4 py-2.5 text-sm border border-slate-300 focus:border-brand-lime focus:ring-1 focus:ring-brand-lime focus:outline-none rounded-none bg-slate-50 transition-colors" placeholder="Adresse ou lieu exact" value="{{ old('lieu', $evenement->lieu) }}">
                  </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                ...
                <input type="date" name="date_debut" required class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-brand-lime focus:ring-1 focus:ring-brand-lime focus:outline-none rounded-none bg-slate-50 transition-colors" value="{{ old('date_debut', \Carbon\Carbon::parse($evenement->date_debut)->format('Y-m-d')) }}">
              ...
              <div>
                ...
                <input type="date" name="date_fin" required class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-brand-lime focus:ring-1 focus:ring-brand-lime focus:outline-none rounded-none bg-slate-50 transition-colors" value="{{ old('date_fin', \Carbon\Carbon::parse($evenement->date_fin)->format('Y-m-d')) }}">
              ...
              <div>
                ...
                <input type="time" name="heure" required class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-brand-lime focus:ring-1 focus:ring-brand-lime focus:outline-none rounded-none bg-slate-50 transition-colors" value="{{ old('heure', \Carbon\Carbon::parse($evenement->heure)->format('H:i')) }}">
    ...
    <div class="bg-white border border-slate-200 shadow-sm p-6 relative overflow-hidden mt-6">
      ...
      <textarea name="description" rows="6" class="w-full px-4 py-3 text-sm border border-slate-300 focus:border-brand-lime focus:ring-1 focus:ring-brand-lime focus:outline-none rounded-none bg-slate-50 transition-colors" placeholder="Détaillez le programme, les intervenants, etc...">{{ old('description', $evenement->description) }}</textarea>
    ...
    <div class="bg-white border border-slate-200 shadow-sm p-6 relative overflow-hidden mt-6" x-data="multipleImageUploader()">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 mt-6" x-show="existingImages.length > 0 || files.length > 0" x-cloak>
        ...
        <template x-for="(file, index) in files" :key="'new-'+index">
          <div class="relative group aspect-square border border-brand-lime bg-slate-100 shadow-sm overflow-hidden">
            ...
            <div class="absolute top-1 left-1 bg-brand-lime text-slate-900 text-[9px] font-bold px-1.5 py-0.5 uppercase tracking-wider">Nouvelle</div>
          </div>
        </template>
      </div>
    </div>
