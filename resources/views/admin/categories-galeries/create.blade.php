...
@section('title', 'Créer Catégorie Galerie | AssoCulture')
...
<div class="space-y-6">
  ...
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-slate-900 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">CRÉER UNE CATÉGORIE</h1>
      <p class="text-slate-400 text-sm mt-0.5">Ajoutez une nouvelle catégorie à la médiathèque.</p>
    </div>
  ...
  <div class="bg-white border border-slate-200 shadow-sm rounded-none">
    <form action="{{ route('dashboard.admin.categories-galeries.store') }}" method="POST" class="p-6 space-y-6">
      @csrf
      ...
      <div>
        ...
        <input type="text" name="libelle" value="{{ old('libelle') }}" required class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none">
      ...
      <div>
        ...
        <textarea name="description" rows="3" class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none" placeholder="Description de la catégorie...">{{ old('description') }}</textarea>
      ...
      <div>
        ...
        <select name="actif" required class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none bg-white">
          <option value="OUI" {{ old('actif', 'OUI') == 'OUI' ? 'selected' : '' }}>Actif</option>
          <option value="NON" {{ old('actif') == 'NON' ? 'selected' : '' }}>Inactif</option>
        </select>
