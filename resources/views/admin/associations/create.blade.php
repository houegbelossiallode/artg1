...
@section('title', 'Nouvelle Association | AssoCulture')
...
<div class="space-y-6">
  ...
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-brand-lime flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <div class="flex items-center gap-2 text-xs text-slate-400 mb-1">
        ...
        <span>/</span>
        <span class="text-slate-700 font-bold">Nouvelle Association</span>
      </div>
      <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">NOUVELLE FICHE ASSOCIATION</h1>
      <p class="text-slate-400 text-sm mt-0.5">Enregistrez la présentation, la mission, la vision et les coordonnées du siège.</p>
    </div>
  ...
  <form action="{{ route('dashboard.admin.associations.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf

    {{-- ══ IDENTITÉ & LOGO ══ --}}
    <div class="bg-white border border-slate-200 shadow-sm">
      <div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center gap-3">
        <div class="w-7 h-7 bg-brand-lime flex items-center justify-center shrink-0">
      ...
      <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input type="text" name="nom" value="{{ old('nom') }}" required
            class="w-full px-3 py-2 bg-slate-50 border border-slate-300 text-sm focus:outline-none focus:border-brand-lime transition @error('nom') border-red-400 bg-red-50 @enderror"
            placeholder="ex : AssoCulture Bénin">
          @error('nom')<p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
          <label class="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1.5">Logo</label>
          <input type="file" name="logo" accept="image/*"
            class="w-full px-3 py-1.5 text-xs bg-slate-50 border border-slate-300 focus:outline-none focus:border-brand-lime transition cursor-pointer">
      ...
      <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <input type="email" name="email" value="{{ old('email') }}" required
            class="w-full px-3 py-2 bg-slate-50 border border-slate-300 text-sm focus:outline-none focus:border-brand-lime transition @error('email') border-red-400 bg-red-50 @enderror"
            placeholder="contact@assoculture.org">
        ...
        <div>
          ...
          <input type="text" name="telephone" value="{{ old('telephone') }}" required
            class="w-full px-3 py-2 bg-slate-50 border border-slate-300 text-sm focus:outline-none focus:border-brand-lime transition @error('telephone') border-red-400 bg-red-50 @enderror"
        ...
        <div>
          ...
          <input type="text" name="adresse" value="{{ old('adresse') }}" required
            class="w-full px-3 py-2 bg-slate-50 border border-slate-300 text-sm focus:outline-none focus:border-brand-lime transition @error('adresse') border-red-400 bg-red-50 @enderror"
      ...
      <div class="p-6 space-y-4">
        <div>
          ...
          <textarea name="description" rows="3"
            class="w-full px-3 py-2 bg-slate-50 border border-slate-300 text-sm focus:outline-none focus:border-brand-lime transition resize-none"
            placeholder="Présentation synthétique de l'association...">{{ old('description') }}</textarea>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <textarea name="mission" rows="4"
              class="w-full px-3 py-2 bg-slate-50 border border-slate-300 text-sm focus:outline-none focus:border-brand-lime transition resize-none"
              placeholder="Notre mission...">{{ old('mission') }}</textarea>
          </div>
          <div>
            ...
            <textarea name="vision" rows="4"
              class="w-full px-3 py-2 bg-slate-50 border border-slate-300 text-sm focus:outline-none focus:border-brand-lime transition resize-none"
              placeholder="Notre vision...">{{ old('vision') }}</textarea>
          </div>
          <div>
            ...
            <textarea name="historique" rows="4"
              class="w-full px-3 py-2 bg-slate-50 border border-slate-300 text-sm focus:outline-none focus:border-brand-lime transition resize-none"
              placeholder="Notre histoire...">{{ old('historique') }}</textarea>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex items-center justify-end gap-3 pb-4">
      ...
      <button type="submit"
        class="px-6 py-2 bg-brand-lime hover:brightness-105 text-slate-900 font-bold text-xs uppercase tracking-widest transition shadow-sm flex items-center gap-2">
