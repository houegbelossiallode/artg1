...
@section('title', 'Créer Élément Galerie | AssoCulture')
...
<div class="space-y-6">
  ...
  <div class="bg-white border border-slate-200 shadow-sm px-6 py-5 border-l-4 border-l-slate-900 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
    <div>
      <h1 class="text-lg font-bold text-slate-900 uppercase tracking-wide">AJOUTER UN ÉLÉMENT</h1>
      <p class="text-slate-400 text-sm mt-0.5">Ajoutez une photo, vidéo ou création à la médiathèque.</p>
    </div>
  ...
  <div class="bg-white border border-slate-200 shadow-sm rounded-none">
    <form action="{{ route('dashboard.admin.galeries.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
      @csrf
      ...
      <div>
        ...
        <input type="text" name="titre" value="{{ old('titre') }}" required class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none">
      ...
      <div>
        ...
        <textarea name="description" rows="3" class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none" placeholder="Description de l'élément...">{{ old('description') }}</textarea>
      ...
      <div>
        ...
        <select name="categorie_galerie_id" required class="w-full px-4 py-2.5 text-sm border border-slate-300 focus:border-slate-900 focus:outline-none rounded-none bg-white">
          ...
          @foreach($categories as $cat)
            <option value="{{ $cat->id }}" {{ old('categorie_galerie_id') == $cat->id ? 'selected' : '' }}>{{ $cat->libelle }}</option>
          @endforeach
        </select>
      ...
      <div>
        ...
        <p class="text-xs text-slate-400 mt-1">Formats acceptés: JPG, PNG, GIF, MP4, WebM, MP3, WAV. Max 10 Mo.</p>
      ...
      <div class="flex justify-end gap-3 pt-6 border-t border-slate-200">
        ...
        <button type="submit" class="px-6 py-2.5 text-xs uppercase font-bold tracking-wider bg-slate-900 hover:bg-slate-800 text-amber-500 transition-colors rounded-none">Enregistrer</button>
      </div>
    </form>
  </div>
